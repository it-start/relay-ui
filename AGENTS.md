...
- Run `bun run build` to ensure both the Vite client and the esbuild server bundle succeed. Node cannot run `server.ts` directly — `./server/store` is a directory import and ESM will not resolve it — so the bundle is what production runs.
